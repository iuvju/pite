...
/**
 ...
 2013-05-05  崎轩（chuncheng.li@taobao.com）:

 - add JuSkrollr 在工厂模式外包装了一个多实例的方法
 - cancel skrollr
 		无法使用该变量，此版本不能使用skrollr.ie.js文件
 		调用方法由以前的skrollr.init(options)
 		变成
 			new JuSkroll(options);
 - add options config
		配置选项增加项： container 层的id值
		eg:
			HTML :
				<div id="panel"></div>
			JavaScript :
				var ju = new JuSkrollr({
					container: 'pannel'
				});

 API change log:
 - add .stop() methods 使当前层的动画暂停，并保存当前层的暂停帧
 - add .resume() metods 使当前层的动画从保存位置开始继续
 */